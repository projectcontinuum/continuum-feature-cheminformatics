...
/**
 * Remove Hydrogens Node Model
 *
 * Removes explicit hydrogens from molecules represented as SMILES strings using RDKit.
 * For each row in the input table, the node reads a SMILES string from the configured column,
 * parses it into an RDKit molecule object, removes explicit hydrogens via RDKFuncs.removeHs(),
 * and re-serializes the result as a SMILES string. Invalid SMILES produce an empty string
 * rather than an error, so the node never crashes on malformed input.
 *
 * **Input Ports:**
 * - `input`: Input table containing a column with SMILES strings
 *
 * **Output Ports:**
 * - `output`: Table with all original columns plus a new column containing SMILES without explicit Hs
 *
 * **Configuration Properties:**
 * - `smilesColumn` (required): Name of the column containing SMILES strings
 * - `newColumnName` (optional, default "smiles_no_hs"): Name for the new output column
 ...
 */
@ContinuumNode
class RemoveHsNodeModel : ProcessNodeModel() {
    companion object {
        private val LOGGER = LoggerFactory.getLogger(RemoveHsNodeModel::class.java)
        ...
        """
        {
          ...
          "properties": {
            ...
            "newColumnName": {
              ...
              "description": "Name for the new column containing SMILES without explicit hydrogens",
              "default": "smiles_no_hs"
            },
            "removeSourceColumn": {
              "type": "boolean",
    ...
    override val metadata = ContinuumWorkflowModel.NodeData(
        ...
        description = "Removes explicit hydrogens from SMILES strings using RDKit",
        title = "Remove Hydrogens",
        subTitle = "Remove explicit Hs from molecules via RDKit",
    ...
    override fun execute(
        ...
        val newColumnName = properties["newColumnName"]?.toString() ?: "smiles_no_hs"
        val removeSourceColumn = properties["removeSourceColumn"] as? Boolean ?: false

        LOGGER.info("Removing Hs from SMILES in column '$smilesColumn' into '$newColumnName' (removeSource=$removeSourceColumn)")
        ...
        nodeOutputWriter.createOutputPortWriter("output").use { writer ->
            inputReader.use { reader ->
                ...
                while (row != null) {
                    ...
                    // Parse SMILES, remove Hs, and convert back to SMILES
                    var resultSmiles = ""
                    if (smilesValue.isNotEmpty()) {
                        resultSmiles = RDKitNodeHelper.withMolecule(smilesValue) { mol ->
                            val molNoHs = RDKFuncs.removeHs(mol)
                            try {
                                RDKFuncs.MolToSmiles(molNoHs)
                            } finally {
                                molNoHs.delete()
                            }
                        } ?: ""
        ...
        LOGGER.info("Remove Hs conversion completed: processed rows")
    }
}
