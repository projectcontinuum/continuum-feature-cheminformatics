...
/**
 * Aromatize Node Model
 *
 * Applies aromaticity perception to molecules represented as SMILES strings using RDKit.
 * For each row in the input table, the node reads a SMILES string from the configured column,
 * parses it into an RDKit molecule object, applies aromaticity perception via
 * RDKFuncs.setAromaticity(), and re-serializes the result as a SMILES string. Invalid SMILES
 ...
 *
 * **Output Ports:**
 * - `output`: Table with all original columns plus a new column containing aromatic SMILES
 *
 * **Configuration Properties:**
 * - `smilesColumn` (required): Name of the column containing SMILES strings
 * - `newColumnName` (optional, default "aromatic_smiles"): Name for the new output column
 ...
 */
@ContinuumNode
class AromatizeNodeModel : ProcessNodeModel() {
    companion object {
        private val LOGGER = LoggerFactory.getLogger(AromatizeNodeModel::class.java)
        ...
        """
        {
          ...
          "properties": {
            ...
            "newColumnName": {
              ...
              "description": "Name for the new column containing aromatic SMILES",
              "default": "aromatic_smiles"
            },
            "removeSourceColumn": {
              "type": "boolean",
    ...
    override val metadata = ContinuumWorkflowModel.NodeData(
        ...
        description = "Applies aromaticity perception to SMILES strings using RDKit",
        title = "Aromatize",
        subTitle = "Apply aromaticity perception to molecules via RDKit",
    ...
    override fun execute(
        ...
        val newColumnName = properties["newColumnName"]?.toString() ?: "aromatic_smiles"
        val removeSourceColumn = properties["removeSourceColumn"] as? Boolean ?: false

        LOGGER.info("Aromatizing SMILES from column '$smilesColumn' into '$newColumnName' (removeSource=$removeSourceColumn)")
        ...
        nodeOutputWriter.createOutputPortWriter("output").use { writer ->
            inputReader.use { reader ->
                ...
                while (row != null) {
                    ...
                    // Parse SMILES, apply aromaticity, and convert back to SMILES
                    var resultSmiles = ""
        ...
        LOGGER.info("Aromatize conversion completed: processed rows")
    }
}
