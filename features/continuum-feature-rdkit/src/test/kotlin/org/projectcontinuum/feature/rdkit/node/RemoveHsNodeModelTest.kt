...
class RemoveHsNodeModelTest {

    private lateinit var nodeModel: RemoveHsNodeModel
    ...
    @BeforeEach
    fun setUp() {
        nodeModel = RemoveHsNodeModel(jacksonObjectMapper())
    ...
    fun `test node metadata is properly configured`() {
        ...
        assertEquals("org.projectcontinuum.feature.rdkit.node.RemoveHsNodeModel", metadata.id)
        assertEquals("Removes explicit hydrogens from SMILES strings using RDKit", metadata.description)
        assertEquals("Remove Hydrogens", metadata.title)
        assertEquals("Remove explicit Hs from molecules via RDKit", metadata.subTitle)
    ...
    @Test
    fun `test execute with valid SMILES removes explicit hydrogens`() {
        val rows = listOf(
            mapOf("smiles" to "[H]OC(=O)c1ccccc1", "name" to "benzoic acid with Hs"),
        ...
        assertNotNull(rowCaptor.allValues[0]["smiles_no_hs"])
        assertNotNull(rowCaptor.allValues[1]["smiles_no_hs"])

        // Check original columns are preserved
        assertEquals("[H]OC(=O)c1ccccc1", rowCaptor.allValues[0]["smiles"])
        assertEquals("benzoic acid with Hs", rowCaptor.allValues[0]["name"])
    ...
    fun `test execute with invalid SMILES writes empty string and does not crash`() {
        ...
        assertEquals("", rowCaptor.allValues[0]["smiles_no_hs"])
        ...
        assertNotNull(rowCaptor.allValues[1]["smiles_no_hs"])
        assertTrue((rowCaptor.allValues[1]["smiles_no_hs"] as String).isNotEmpty())
    }
    ...
    fun `test execute uses default newColumnName and removeSourceColumn`() {
        ...
        // Default column name is "smiles_no_hs"
        assertTrue(rowCaptor.allValues[0].containsKey("smiles_no_hs"))
    ...
    fun `test execute with removeSourceColumn true removes the source column`() {
        ...
        assertTrue(outputRow.containsKey("smiles_no_hs"))
    }
