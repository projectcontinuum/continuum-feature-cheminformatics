...
class MoleculeWriterNodeModelTest {

    private lateinit var nodeModel: MoleculeWriterNodeModel
    ...
    @BeforeEach
    fun setUp() {
        nodeModel = MoleculeWriterNodeModel(jacksonObjectMapper())
    ...
    fun `test node metadata is properly configured`() {
        ...
        assertEquals("org.projectcontinuum.feature.rdkit.node.MoleculeWriterNodeModel", metadata.id)
        assertEquals("Converts SMILES strings to different molecule formats using RDKit", metadata.description)
        assertEquals("Molecule Writer", metadata.title)
        assertEquals("Convert SMILES to SMILES, SDF, or SMARTS", metadata.subTitle)
    ...
    @Test
    fun `test execute with SMILES output format`() {
        val rows = listOf(
        ...
        // Check converted SMILES values are present
        assertNotNull(rowCaptor.allValues[0]["converted"])
        assertNotNull(rowCaptor.allValues[1]["converted"])
    ...
    @Test
    fun `test execute with SDF output format`() {
        val rows = listOf(
            mapOf("smiles" to "c1ccccc1", "name" to "benzene")
        )
        mockSequentialReads(mockInputReader, rows)
        ...
        val converted = rowCaptor.allValues[0]["converted"] as String
        assertTrue(converted.isNotEmpty())
        // SDF MolBlock typically contains "V2000" or "V3000"
        assertTrue(converted.contains("V2000") || converted.contains("V3000"))
    }

    @Test
    fun `test execute with SMARTS output format`() {
        val rows = listOf(
            mapOf("smiles" to "c1ccccc1", "name" to "benzene")
        )
        mockSequentialReads(mockInputReader, rows)
    ...
    fun `test execute with invalid SMILES writes empty string and does not crash`() {
        ...
        assertEquals("", rowCaptor.allValues[0]["converted"])
        ...
        // Valid SMILES should produce non-empty output
        assertNotNull(rowCaptor.allValues[1]["converted"])
        assertTrue((rowCaptor.allValues[1]["converted"] as String).isNotEmpty())
    }
    ...
    fun `test execute with removeSourceColumn true removes the source column`() {
        ...
        // Converted column should be present
        assertTrue(outputRow.containsKey("converted"))
    }
    ...
    fun `test execute uses default properties`() {
        ...
        verify(mockPortWriter, times(1)).write(any(), rowCaptor.capture())
        // Default column name is "converted"
        assertTrue(rowCaptor.allValues[0].containsKey("converted"))
    ...
    @Test
    fun `test execute with empty SMILES string writes empty converted`() {
        val rows = listOf(
